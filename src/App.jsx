  ...
  return <div className="page">
    ...
    <div className="shell">
      ...
      {tab === 'reports' && <>
        ...
        <Panel title={report.label}><div className="report-summary-heading">Time Worked</div><div className="stats"><Stat label="Hours Worked" value={fmtHours(report.net)} sub={`${report.daysAtWork} day(s) worked`} /><Stat label="Paid Stat Holidays" value={fmtHours(report.paidStatHours)} sub={`${report.statDays} holiday${report.statDays === 1 ? '' : 's'}`} /><Stat label="Vacation Taken" value={fmtHours(report.vacation)} sub={`${report.vacationDays} day(s) logged`} /></div><div className="report-summary-heading">Banked Time Position</div><div className="stats"><Stat label="Banked Time Taken" value={fmtHours(report.bankedUsed)} sub="Used as paid time" /><Stat label="Banked Time Earned" value={`+${fmtHours(report.bankedEarned)}`} sub="Weekday overtime, stats, and days off" /><Stat label="Banked Time Available" value={fmtHours(allTime.balance)} sub="Available to take later" /></div><div className="readouts"><Readout label="Stat Holidays" value={report.statHolidays.map(holiday => holiday.name).join(' · ') || 'None in this range'} /><Readout label="Range" value={`${formatDate(report.start)} - ${formatDate(report.end)}`} /></div><div className="report-export-actions"><button className="export-btn" onClick={printReport}>Print Report</button><button className="export-btn secondary" onClick={exportCsv}>Export CSV</button></div></Panel>
...
function openPrintableReport(report, allTime, workSettings, generatedAt) {
  ...
  const summary = [
    ['Hours Worked', fmtHours(report.net)],
    ['Paid Stat Holidays', fmtHours(report.paidStatHours)],
    ['Vacation Taken', fmtHours(report.vacation)],
    ['Banked Time Taken', fmtHours(report.bankedUsed)],
    ['Banked Time Earned', `+${fmtHours(report.bankedEarned)}`],
    ['Banked Time Available', fmtHours(allTime.balance)],
...
// Required by main.jsx. Startup errors are also surfaced by main.jsx itself.
